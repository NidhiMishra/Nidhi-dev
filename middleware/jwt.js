...
module.exports = {
	createAccessToken: async (payload, options = {}) => {
		try {
			const token = await jwt.sign(payload, ACCESS_SECRET_TOKEN, options);
			return token;
		} catch (e) {
			console.log(e);
			return false;
	...
	checkAccessToken: async (req, res, next) => {
		try {
			...
			const user = await jwt.verify(token, ACCESS_SECRET_TOKEN);
	...
	createRefreshToken: async (payload, options = {}) => {
		try {
			const token = await jwt.sign(payload, REFRESH_SECRET_TOKEN, options);
			return token;
		} catch (e) {
			console.log(e);
			return false;
	...
	checkRefreshToken: async (token) => {
		try {
			...
			const user = await jwt.verify(token, REFRESH_SECRET_TOKEN);
